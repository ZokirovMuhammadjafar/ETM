...
public class CustomerSQLException extends RuntimeException {
    ...
    private void initMessage() {
        ...
        try {
            friendlyMessage = systemMessage.substring(systemMessage.lastIndexOf("ERROR: ") + 7,
                    systemMessage.indexOf("Where")).trim();
            if (friendlyMessage.isEmpty()) {
                friendlyMessage = systemMessage.substring(systemMessage.lastIndexOf("detail: ") + 8,
                        systemMessage.indexOf("hint: ")).trim();
            }
            if (friendlyMessage.isEmpty()) {
